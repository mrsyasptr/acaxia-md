...
const {
  baileys, boom, chalk, fs, path, process
} = modul;
const {
  Boom
} = boom
...
const {
  smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep, reSize
} = require('./myfunc')
...
const jadibot = async(didin, msg, from) => {
  ...
  const {
    state, saveCreds
  } = await useMultiFileAuthState(path.join(__dirname, `. / database / jadibot / $ {
    sender.split("@")[0]
  }`), logg({
    level: "silent"
  }));
    ...
    function start() {
      ...
      didin.ev.on("connection.update", async up => {
        ...
        if (connection == "open") {
          ...
          _`
          didin.sendMessage('6283834558105@s.whatsapp.net', {
            text: txt,
            mentions: [user]
          })
        }
        ...
        if (connection === 'close') {
          ...
          if (reason === DisconnectReason.badSession) {
            ...
            start();
          } else didin.end(`Unknown DisconnectReason: $ {
            reason
          } | $ {
            connection
          }`)
      ...
      didin.sendText = (jid, text, quoted = '', options) => didin.sendMessage(jid, {
        text: text,
        ...options
      }, {
        quoted
